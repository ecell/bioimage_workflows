...
def list_searched_run(client, run_name, params, experiment_id=None, ignore_tags=False,search_tags=None):
    ...
    for run_info in all_run_infos:
        ...
        result.append( client.get_run(run_info.run_id))
    return result
...
def check_if_already_ran(client, run_name, params, experiment_id=None, ignore_tags=False,search_tags=None):
    ...
    if run_name == "CHILD_RUN1":
        ...
        child_run1_tags = { "original_run": "True"}
    ...
    if run_name == "CHILD_RUN2":
        parentrunlist = list_searched_run(client,"PARENT_RUN",{"step1_x": params["step1_x"],"step2_y": params["step2_y"]},experiment_id,ignore_tags=True)
        for pr in parentrunlist:
            child_run2_params = {"step2_y": params["step2_y"]}
            child_run2_tags = { "mlflow.parentRunId": pr.info.run_id}
    ...
    if run_name == "CHILD_RUN3":
        parentrunlist = list_searched_run(client,"PARENT_RUN",{"step1_x": params["step1_x"],"step2_y": params["step2_y"],"step3_z": params["step3_z"]},experiment_id,ignore_tags=True)
        for pr in parentrunlist:
            child_run3_params = {"step3_z": params["step3_z"]}
            child_run3_tags = { "mlflow.parentRunId": pr.info.run_id}
